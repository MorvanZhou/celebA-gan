...
import numpy as np
from cnn import resnet_g, resnet_d, dc_g, dc_d
...
class ACGAN(keras.Model):
    ...
    def __init__(self, latent_dim, label_dim, img_shape, a=-1, b=1, c=1, summary_writer=None, lr=0.0002, beta1=0.5, beta2=0.999, net="dcnet"):
        ...
        self.loss_class = keras.losses.BinaryCrossentropy(from_logits=True, reduction="none")
    ...
    def train_g(self, random_img_label):
        ...
        if self._train_step % 300 == 0 and self.summary_writer is not None:
            with self.summary_writer.as_default():
                ...
                tf.summary.scalar("g/sigmoid", tf.reduce_mean(loss_class), step=self._train_step)
    ...
    def step(self, real_img, real_img_label):
        random_img_label = tf.convert_to_tensor(
            np.random.choice([0, 1], (len(real_img)*2, self.label_dim), replace=True), dtype=tf.int32)
